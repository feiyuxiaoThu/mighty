...
def main():
    ...
    for bag_dir in bag_dirs:
        ...
        last_part = bag_path.split("/")[-1]
        bag_path = os.path.join(bag_path, last_part)
        ...
        try:
            ...
            # print in green if no collision, red if collision
            if collision:
                print(f"\033[91mCollision detected in {bag_dir}\033[0m")
            else:
                print(f"\033[92mNo collision in {bag_dir}\033[0m")
        except Exception as e:
            print(f"Error processing bag {bag_dir}: {e}")
            results[bag_dir] = "error"

    # Write results to collision_check.txt in the same folder
    output_file = os.path.join(bag_folder, "dynamic_collision_check.txt")

...
def generate_launch_description():
    ...
    # global planner
    global_planner_arg = DeclareLaunchArgument('global_planner', default_value='sjps', description='Global planner to use')
    ...
    def launch_setup(context, *args, **kwargs):
        ...
        if not use_quadruped:
            if use_hardware and use_onboard_localization:
                nodes_to_start = [dynus_node, odom_to_state_node] # use T265 for localization
            elif use_hardware and not use_onboard_localization:
                nodes_to_start = [dynus_node, pose_twist_to_state_node] # use Vicon for localization
            else:
                nodes_to_start = [dynus_node, robot_state_publisher_node, spawn_entity_node, fake_sim_node] # simulation
        else:
            nodes_to_start = [dynus_node, quadruped_odom_to_state_node]

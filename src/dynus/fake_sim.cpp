...
public:
    ...
    FakeSim() : Node("fake_sim"), br_(this, rclcpp::QoS(10).reliable().durability_volatile()) // Note: adding rclcpp::QoS(10).reliable().durability_volatile() to tf broadcaster helped a lot with the visualization lag.
    {
        ...
        // Timer to periodically look up tf data and publish state
        if (use_ground_robot_)
            tf2_timer_ = this->create_wall_timer(10ms, std::bind(&FakeSim::updateStateFromTF, this));
...
private:
    ...
    void updateStateFromTF()
    {
        try
        {
            ...
            // Ground is detected as obstacles in octomap, so DYNUS is planning at default_goal_z_ altitude, so we set the z position to default_goal_z_ here
    ...
    void pubCallback()
    {
        ...
        if (send_state_to_gazebo_ && !use_ground_robot_)
        {
            // threads_.push_back(std::thread(&FakeSim::sendGazeboState, this));
            std::thread(&FakeSim::sendGazeboState, this).detach();
        }

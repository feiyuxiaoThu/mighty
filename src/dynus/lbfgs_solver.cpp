...
void SolverLBFGS::initializeSolver(const planner_params_t &params)
{
    ...
    mass_ = params.mass;
    g_ = params.g;
    seam_min_dist_ = params.seam_min_dist;
}
                                                        ...
                                                        double smooth_eps)
{
    ...
    // --- prune nearly coincident interior waypoints (seams) and drop the
    //     corresponding corridor cell to keep sizes aligned -----------------
    const double seam_min_dist = seam_min_dist_; // heuristic
    ...
    {
        Vec3 a = colAsVec3(i);
        Vec3 b = colAsVec3(i + 1);
        const double gap = (b - a).norm();

        if (gap < seam_min_dist)
        {
            // Drop seam 'i' and the corridor cell to its right (segment index i)
            if (i < (int)A_stat_.size())
            {
                A_stat_.erase(A_stat_.begin() + i);
                b_stat_.erase(b_stat_.begin() + i);
            }
            // Do not push 'a'; re-check new neighbor against next point.
            ++i;
            continue;
        }
